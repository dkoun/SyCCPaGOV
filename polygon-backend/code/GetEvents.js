...
async function readContractAddress() {
    try {
        const jsonString = await fs.readFile('code/build/contracts/MyToken.json', 'utf8');
        const data = JSON.parse(jsonString);
        const addr = data.networks["80001"].address;
        return addr;
    } catch (err) {
        console.log("Error:", err);
    }
...
async function searchEvents(from) {
    ...
    const providerUrl = 'https://polygon-mumbai.infura.io/v3/362babd917ae42ae8bfd18788c16a259';
